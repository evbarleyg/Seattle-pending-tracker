...
(function initBuyerProfile(rootFactory) {
  const api = rootFactory();
  if (typeof module !== "undefined" && module.exports) {
    module.exports = api;
  }
  if (typeof window !== "undefined") {
    window.BuyerProfile = api;
  }
})(function buildBuyerProfile() {
  const DAY_MS = 24 * 60 * 60 * 1000;
  const STORAGE_KEY = "buyer_lens_profile_memory_v1";
  const MICRO_GROUPS = [
    "Ballard",
  ...
  const DEFAULT_PROFILE_MEMORY = {
    ...
    sampleAddresses: [
      ...
      "2616 Franklin Avenue E, Seattle, WA 98102",
      "7544 31st Avenue NW, Seattle, WA 98117",
      "5625 7th Avenue NW, Seattle, WA 98107",
      "339 NW 77th Street, Seattle, WA 98117",
      "6707 6th Avenue NW, Seattle, WA 98117",
      "4146 2nd Avenue NW, Seattle, WA 98107",
      "406 N Bowdoin Place, Seattle, WA 98103",
      "2419 2nd Avenue W, Seattle, WA 98119",
    ],
    watchlistGroups: MICRO_GROUPS.slice(),
    flexNeighborhoodLabels: [
  ...
  function normalizeProfileMemory(rawProfile) {
    const source = rawProfile && typeof rawProfile === "object"
      ? rawProfile
    ...
    return {
      ...
      traits: normalizeStringList(source.traits).length
        ? normalizeStringList(source.traits)
        : DEFAULT_PROFILE_MEMORY.traits.slice(),
  ...
  function profileScore(row, rawProfile) {
    ...
    if (safeNumber(row?.closePrice) && safeNumber(row?.closePrice) >= 900000 && safeNumber(row?.closePrice) <= 1900000) {
      score += 2;
    }
  ...
  function rowMatchesProfile(row, rawProfile) {
    const profile = normalizeProfileMemory(rawProfile);
    const pulseGroup = String(row?.pulseWatchlistGroup || "").trim();
    const neighborhoodLabel = String(row?.neighborhoodLabel || row?.neighborhood || "").trim();
    const typeLabel = String(row?.typeLabel || row?.type || "").trim();
    ...
    return hasLocationFit
      && typeLabel === profile.typeLabel
      && profileScore(row, profile) >= 60;
  }
  ...
  function summarizeCompetition(rows) {
    ...
    return {
      ...
      medianSaleToList: median(ratioValues),
      medianBidUp: median(bidValues),
      medianClosePrice: median(closeValues),
    };
  }

  function buildProfileCohort(rows, rawProfile) {
    const profile = normalizeProfileMemory(rawProfile);
  ...
  function fitLabel(profileShare) {
  ...
  function buildMicromarketProfiles(rows, rawProfile, nowValue) {
    const profile = normalizeProfileMemory(rawProfile);
